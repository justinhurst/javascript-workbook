...
const nums = strNums.map((item,index)=>{
  return Number(item)
})

// Find the sum of the even values
// filter returns items that pass a test
const sumEvens = nums.filter( item => item % 2 == 0 ).reduce((total, num) => {
  return total + num
})

// Find the index of the first "Austin" value (the value plus its index equals 512)
// findIndex returns the index of the first occurence of something
...
//get an array of the weather_state_names, then filter out items that are not unique
const weatherStates = weather.map(item => item.weather_state_name)
  .filter((item, index, arr) => arr.indexOf(item) === index);
console.log(weatherStates);

//find the id of the object in weather that has a min_temp of 15.915
//find returns the first occurence of something
//test for the min temperature that we want
